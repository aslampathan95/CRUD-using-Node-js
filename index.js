...
app.get("/products", (req, res) => {
  // let sql = "SELECT * FROM products limit 5 offset 0";
  ...
  connection.query(sql, (err, result) => {
        ...
        connection.query(sql, (err, result)=>{
            ...
            // res.json(result)
            res.render('index', {data: result, page, iterator, endingLink, numberOfPages, title: "PRODUCTS",users: result,});
        });
    });
});
...
app.post("/save/category", (req, res) => {
  ...
  let sql = `INSERT INTO categories(CategoryName) values('${data.CategoryName}')`;
  let query = connection.query(sql, (err, results) => {
    if (err) throw err;
    res.redirect("/");
  });
